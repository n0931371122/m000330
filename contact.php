<!doctype html>
<html lang="zh">

<head>
    <?php include "header-title.php"; ?>

</head>

<body>
    <div class="wp" id="contact">
        <?php include "header.php"; ?>
        <!-- main ======================================================================================= -->
        <main>
            <h1 class="hidden">聯絡我們</h1>
            <div class="blank footer-space overflow-hidden pt-5" style="background-image:url(styles/images/new/bg.jpg)">
                <div class="editor max1400 pt-sm-5 mt-lg-5 mb-4 mb-lg-5 pb-2" id="contactEditor">
                    <!--編輯器開始-->
                    <div class="contact-info position-relative row pt-3 pt-md-5 mt-md-4">
                        <div class="dec-img col-lg-6 col-xl-7" data-aos="fade-up">
                            <img src="styles/images/contact/img.png" alt="華元食品">
                        </div>
                        <div class="col-12 col-lg-6 col-xl-5" data-aos="fade-up">
                            <span class="d-block font-90 font-weight-800 lh1 mb-2 mb-md-4">
                                <span class="d-block text-gradient-red">
                                    <span>LET'S</span>
                                </span>
                                <span class="d-block text-gradient-red">
                                    <span>TALK TO US</span>
                                </span>
                            </span>
                            <span class="d-block font-36 font-weight-bold">聯絡我們</span>
                            <ul class="noneStyle mt-4 pt-1">
                                <li class="mb-2 pb-1">
                                    <div class="d-flex font-18">
                                        <span class="circle flex-center text-white bg-purple rounded-circle flex-shrink-0 mr-3">
                                            <i class="icon-phone"></i>
                                        </span>
                                        <div class="pt-2">
                                            <span class="d-block mb-1">
                                                <span class="text-purple">消費者專線</span>
                                                <span class="mx-1">/</span>
                                                <a class="font-weight-bold text-hover-red" href="tel:+886-800-234123">0800-234123</a>
                                            </span>
                                            <span class="d-block text-gray808080">(週一 ~ 週五08:00~17:00，如未接通請留言)</span>
                                        </div>
                                    </div>
                                </li>
                                <li class="mb-2 pb-1">
                                    <div class="font-18 flex-align-center">
                                        <span class="circle flex-center text-white bg-purple rounded-circle flex-shrink-0 mr-3">
                                            <i class="icon-fax"></i>
                                        </span>
                                        <div>
                                            <span class="d-block">
                                                <span class="text-purple">傳真</span>
                                                <span class="mx-1">/</span>
                                                <span class="font-weight-bold">04-26392780</span>
                                            </span>
                                        </div>
                                    </div>
                                </li>
                                <li class="mb-2 pb-1">
                                    <div class="font-18 flex-align-center">
                                        <span class="circle flex-center text-white bg-purple rounded-circle flex-shrink-0 mr-3">
                                            <i class="icon-mail"></i>
                                        </span>
                                        <div>
                                            <span class="d-block">
                                                <span class="text-purple">電子郵件</span>
                                                <span class="mx-1">/</span>
                                                <a class="font-weight-bold text-hover-red" href="mailto:designer.hyfoods@gmail.com"> designer.hyfoods@gmail.com</a>
                                            </span>
                                        </div>
                                    </div>
                                </li>
                                <li class="mb-2 pb-1">
                                    <div class="font-18 flex-align-center">
                                        <span class="circle flex-center text-white bg-purple rounded-circle flex-shrink-0 mr-3">
                                            <i class="icon-location"></i>
                                        </span>
                                        <div>
                                            <span class="d-block">
                                                <span class="text-purple">地址</span>
                                                <span class="mx-1">/</span>
                                                <span>
                                                    <span class="font-weight-bold">台中市梧棲區自立二街392號 </span>
                                                    <a class="text-gray808080 text-hover-red" href="https://www.google.com.tw/maps/place/435%E5%8F%B0%E4%B8%AD%E5%B8%82%E6%A2%A7%E6%A3%B2%E5%8D%80%E8%87%AA%E7%AB%8B%E4%BA%8C%E8%A1%97392%E8%99%9F/@24.2397616,120.5259499,17z/data=!3m1!4b1!4m5!3m4!1s0x346914d5558a2df9:0xe188ff667f50e9c0!8m2!3d24.2397567!4d120.5281386" target="_blank">(Goolge Map)</a>
                                                </span>

                                            </span>
                                        </div>
                                    </div>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <!--編輯器結束-->
                </div>
                <form action="success" class="bg-white max1400 rounded-20 py-4 py-md-5 px-4 px-lg-5 mb-5">
                    <div class="px-xl-3 pt-xl-1">
                        <div class="border-bottom-E2E2E2 pb-1 mb-4" data-aos="fade-up">
                            <p class="font-18 les06 pb-lg-3">歡迎蒞臨「華元食品官方網站」，如果您對產品和公司有任何興趣或疑問，請與我們聯繫，我們將盡快回覆您。 (<span class="text-red">※</span>為必填欄位)</p>
                        </div>
                        <div class="row g-2 pt-lg-3">
                            <div class="col-12 col-md-6 mb-4">
                                <div class="bg-F7F7F7 rounded-30 px-md-2" data-aos="fade-up">
                                    <div class="form-field d-block d-md-flex flex-align-center py-2 py-md-3 px-3 px-md-4">
                                        <label class="d-table d-md-flex flex-justify-space-between font-weight-bold mr-4 pr-1 mb-2 mb-md-0" for="name">
                                            <span>姓名</span>
                                            <span class="ml-2 ml-md-0 text-red">※</span>
                                        </label>
                                        <input class="border-0 lh165 flex-grow px-0" type="text" id="name" name="name" required>
                                    </div>
                                </div>
                            </div>
                            <div class="col-12 col-md-6 mb-4">
                                <div class="bg-F7F7F7 rounded-30 px-md-2" data-aos="fade-up">
                                    <div class="form-field d-block d-md-flex flex-align-center py-2 py-md-3 px-3 px-md-4">
                                        <label class="d-table d-md-flex flex-justify-space-between font-weight-bold mr-4 pr-1 mb-2 mb-md-0" for="company">
                                            <span>公司名稱 </span>
                                            <span class="ml-2 ml-md-0 text-red">※</span>
                                        </label>
                                        <input class="border-0 lh165 flex-grow px-0" type="text" id="company" name="company" required>
                                    </div>
                                </div>
                            </div>
                            <div class="col-12 col-md-6 mb-4">
                                <div class="bg-F7F7F7 rounded-30 px-md-2" data-aos="fade-up">
                                    <div class="form-field d-block d-md-flex flex-align-center py-2 py-md-3 px-3 px-md-4">
                                        <label class="d-table d-md-flex flex-justify-space-between font-weight-bold mr-4 pr-1 mb-2 mb-md-0" for="mail">
                                            <span>E-mail</span>
                                            <span class="ml-2 ml-md-0 text-red">※</span>
                                        </label>
                                        <input class="border-0 lh165 flex-grow px-0" type="email" id="mail" name="mail" required>
                                    </div>
                                </div>
                            </div>
                            <div class="col-12 col-md-6 mb-4">
                                <div class="bg-F7F7F7 rounded-30 px-md-2" data-aos="fade-up">
                                    <div class="form-field d-block d-md-flex flex-align-center py-2 py-md-3 px-3 px-md-4">
                                        <label class="d-table d-md-flex flex-justify-space-between font-weight-bold mr-4 pr-1 mb-2 mb-md-0" for="phone">
                                            <span>聯絡電話</span>
                                            <span class="ml-2 ml-md-0 text-red">※</span>
                                        </label>
                                        <input class="border-0 lh165 flex-grow px-0" type="text" id="phone" name="phone" required>
                                    </div>
                                </div>
                            </div>
                            <div class="col-12 mb-4">
                                <div class="bg-F7F7F7 rounded-30 px-md-2 position-relative z-1" data-aos="fade-up">
                                    <div class="form-field d-block d-md-flex flex-align-center py-2 py-md-3 px-3 px-md-4">
                                        <label class="d-table d-md-flex flex-justify-space-between font-weight-bold mr-4 pr-1 mb-2 mb-md-0" for="issue">
                                            <span>主旨</span>
                                            <span class="ml-2 ml-md-0 text-red">※</span>
                                        </label>
                                        <select name="issue" id="issue" class="selectpicker lh165 w-100">
                                            <option value="">商品相關諮詢</option>
                                            <option value="">服務相關諮詢</option>
                                            <option value="">其他相關諮詢</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                            <div class="col-12 mb-4">
                                <div class="bg-F7F7F7 rounded-30 px-md-2" data-aos="fade-up">
                                    <div class="form-field d-md-flex py-2 py-md-3 px-3 px-md-4">
                                        <label class="d-table d-md-flex flex-justify-space-between font-weight-bold mr-4 pr-1 mb-2 mb-md-0 pt-md-2" for="content">
                                            <span>內容</span>
                                            <span class="ml-2 ml-md-0 text-red">※</span>
                                        </label>
                                        <textarea class="lh165 flex-grow w-100 p-0" name="content" id="content" rows="9" required></textarea>
                                    </div>
                                </div>
                            </div>
                            <div class="col-12 mb-4">
                                <div class="bg-F7F7F7 rounded-30 px-md-2" data-aos="fade-up">
                                    <div class="form-field d-block d-md-flex flex-align-center py-2 px-3 px-md-4">
                                        <label class="d-table d-md-flex flex-justify-space-between font-weight-bold mr-4 pr-1 mb-3 mb-md-0" for="file">
                                            <span>檔案上傳</span>
                                            <span class="ml-2 ml-md-0 text-red">※</span>
                                        </label>
                                        <div class="flex-align-center">
                                            <div class="upload-icon flex-center rounded-5 mr-3 mb-1 mb-md-0">
                                                <img src="styles/images/contact/upload.svg" alt="華元食品">
                                            </div>
                                            <input class="border-0 lh165 flex-grow px-0" type="file" id="file" name="file" required>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-12 row mb-4 mx-0">
                                <div class="col-12 col-md-9 px-0">
                                    <div class="bg-F7F7F7 rounded-30 px-md-2" data-aos="fade-up">
                                        <div class="form-field d-block d-md-flex flex-align-center py-2 px-3 px-md-4">
                                            <label class="d-table d-md-flex flex-justify-space-between font-weight-bold mr-4 pr-1 mb-2 mb-md-0" for="code">
                                                <span>驗證碼</span>
                                                <span class="ml-2 ml-md-0 text-red">※</span>
                                            </label>
                                            <div class="d-flex flex-grow">
                                                <input class="flex-grow w-0" type="text" id="code" name="code" required>
                                                <img src="styles/images/contact/verificationCode.png" alt="華元食品">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-12 col-md-3 pl-0 pl-md-4 pr-0 mt-4 mt-md-0" data-aos="fade-up">
                                    <button type="submit" class="btn-dec font-18 text-white bg-gradient-red rounded-30 w-100 h-100 py-2">
                                        <span class="d-block my-1 my-md-0">送出</span>
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>
                </form>
            </div>
        </main>
        <!-- main ======================================================================================= -->
        <?php include "footer.php"; ?>
    </div>
    <?php include "footer-js.php"; ?>
</body>

</html>